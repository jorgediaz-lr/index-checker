...
import java.util.List;
	...
	public abstract class ModelImpl implements Model {
			...
			if(primaryKeyMultiAttribute == null) {
				...
				if(aux.indexOf('#') > 0) {
					...
					for(int i=0; i < primaryKeyMultiAttribute.length; i++) {
						primaryKeyMultiAttribute[i] = primaryKeyMultiAttribute[i].trim();
					}
				}
				else {
		...
		public boolean hasGroupId() {
			return this.modelExtendsClass(GroupedModel.class) || this.hasAttribute("groupId");
		}
		...
		public List<?> executeDynamicQuery(DynamicQuery dynamicQuery) throws Exception {
			...
			if(statuses != null) {
				dynamicQuery.add(PropertyFactoryUtil.forName("status").in(statuses));
